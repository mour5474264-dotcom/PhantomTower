...
import { Check, Download, FolderOpen, Grid2X2, RefreshCw, Sparkles } from 'lucide-vue-next'
...
const savedAt = ref('')
const settings = reactive({ schemaVersion: 2, preferences: { homeSize: 'standard', canvas: { imageCount: '3', size: '1:1', quality: 'auto', background: 'lines' } }, storage: { exportDir: '' } })
...
function apply(data = {}) {
  ...
  settings.preferences = { ...(data.preferences || {}) }; settings.preferences.canvas = { imageCount: '3', size: '1:1', quality: 'auto', background: 'lines', ...(data.preferences?.canvas || {}) }; settings.storage = { exportDir: '', ...(data.storage || {}) }
}
...
async function save(silent = false) {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('sample-factory-canvas-settings-changed', { detail: settings.preferences.canvas }))
...
onMounted(() => {
  ...
  if (window.phantomTowerUpdate) removeUpdateListener = window.phantomTowerUpdate.on(({ type, data }) => {
    ...
    if (type === 'downloaded') { updateState.value = 'ready'; updateVersion.value = data; updateMessage.value = `版本 ${data} 已下载，点击“立即安装”后应用会退出并完成更新。` }
  ...
  <section class="page settings-page" :class="{ 'is-loading': loading }">
    ...
    <div class="settings-layout">
      <main class="settings-main">
        ...
        <section class="settings-block canvas-block"><div class="block-heading"><div class="section-icon teal"><Grid2X2 :size="18" /></div><div><h3>无限画布</h3><p>用于新建画布和未单独设置参数的生成节点；修改后立即对画布生效。</p></div></div><div class="canvas-fields"><label>默认出图数量<select v-model="settings.preferences.canvas.imageCount" @change="save(true)"><option v-for="count in 8" :key="count" :value="String(count)">{{ count }} 张</option></select></label><label>默认画面比例<select v-model="settings.preferences.canvas.size" @change="save(true)"><option value="1:1">1:1 方形</option><option value="4:3">4:3 横向</option><option value="3:4">3:4 竖向</option><option value="16:9">16:9 宽屏</option><option value="9:16">9:16 竖屏</option></select></label><label>默认清晰度<select v-model="settings.preferences.canvas.quality" @change="save(true)"><option value="auto">自动</option><option value="low">1K</option><option value="medium">2K</option><option value="high">4K</option></select></label><label>新画布背景<select v-model="settings.preferences.canvas.background" @change="save(true)"><option value="lines">网格线</option><option value="dots">点阵</option><option value="blank">纯色</option></select></label></div><div class="field-note"><span class="dot"></span>已有画布的背景和节点参数保持不变</div></section>
        <section class="settings-block updates-block"><div class="block-heading"><div class="section-icon violet"><Sparkles :size="18" /></div><div><h3>自动更新</h3><p>{{ updateMessage }}</p></div><span class="status-pill">{{ updateState === 'latest' ? '已是最新' : updateState === 'error' ? '检查失败' : updateState === 'checking' ? '检查中' : updateState === 'downloading' ? `${updateProgress}%` : updateState === 'ready' ? '可安装' : updateState === 'manual' ? '有新版本' : '待检查' }}</span></div><div class="update-row"><div><b>{{ updateVersion ? `版本 ${updateVersion}` : '保持应用最新' }}</b><small>Windows 下载完成后点击“立即安装”；macOS 将打开 DMG，由你覆盖安装。</small></div><button v-if="['manual','ready'].includes(updateState)" type="button" class="update-button" @click="installUpdate"><Download :size="15" />{{ updateState === 'ready' ? '立即安装' : '打开安装包' }}</button><button v-else type="button" class="update-button" :disabled="updateState === 'checking' || updateState === 'downloading'" @click="checkForUpdate"><RefreshCw :size="15" />检查更新</button></div></section>
      </main>
    </div>
  </section>
</template>
...
.section-icon.teal{color:#167d70;background:#e8f6f2}.canvas-fields{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:20px 0 0 46px}.canvas-fields label{display:grid;gap:6px;color:#516058;font-size:11px;font-weight:700}.canvas-fields select{width:100%;height:36px;border:1px solid #cbd6d0;border-radius:4px;padding:0 8px;color:#2a3731;background:#fbfcfb;font:12px inherit}.canvas-fields select:focus{outline:2px solid #a8d8ce;outline-offset:1px;border-color:#168374}@media(max-width:700px){.canvas-fields{grid-template-columns:repeat(2,minmax(0,1fr));margin-left:0}}@media(max-width:420px){.canvas-fields{grid-template-columns:1fr}}
</style>
