...
import {getModels, getPromptTemplates, generateImage, cancelGeneration, exportImages, downloadUrl} from '../api'
...
async function restoreHistoryEdit() {
  ...
  try {
    ...
    Object.keys(materials.value).forEach((key) => { materials.value[key] = [] })
    materials.value.reference = [markRaw({file, name: file.name, size: file.size, previewUrl: URL.createObjectURL(file)})]
    mode.value = 'image'; imageOperation.value = 'edit'; editParent.value = source; size.value = 'reference'
...
<template>
  <section class="intro">
    ...
    <div class="intro-actions">
      <el-button :icon="Plus" @click="clearCurrent" :disabled="running">新建任务</el-button>
    </div>
  </section>
    ...
    <el-form :disabled="running" label-position="top" class="workspace-form">
      <div class="grid">
        ...
        <section class="panel config-panel">
          <span class="eyebrow">02 / 输出与模型</span>
          <h3>生成配置</h3>
          <div class="config-field-head"><span>模型版本</span><el-button text :icon="Refresh" @click="refresh" :disabled="running">刷新模型</el-button></div>
          <el-form-item>
            <el-select v-model="model" class="studio-select" filterable placeholder="选择模型"
                        popper-class="studio-select-popper" clearable>
          ...
          <p v-if="false && preset" class="preset-preview">{{ preset.systemPrompt }}</p>
          ...
          <div class="task-action-bar">
            <el-button class="clear-button" @click="clearCurrent" :disabled="running">清空当前</el-button>
            <el-button v-if="!running" type="primary" :icon="VideoPlay" @click="generate">开始生成</el-button>
            <!-- This must not inherit el-form's disabled state while a request is active. -->
            <button v-else type="button" class="stop-generation-button" @click="stop">
