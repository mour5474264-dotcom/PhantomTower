...
function InfiniteCanvasPage() {
    ...
    const applyAgentOps = useCallback(() => null, []);
    ...
    // Restoring IndexedDB media is asynchronous. Keep the application chrome
    // stable during that short interval instead of replacing the whole canvas
    // with a skeleton that looks like a browser refresh.
    if (!projectLoaded) return <CanvasLoadingState />;
...
function CanvasLoadingState() {
    return (
        <main className="flex h-full min-h-0 items-center justify-center bg-stone-50 text-sm text-stone-500 dark:bg-stone-950 dark:text-stone-400">
            <span className="rounded-full border border-stone-200 bg-white px-4 py-2 shadow-sm dark:border-stone-800 dark:bg-stone-900">正在恢复画布…</span>
        </main>
    );
}
