...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob> {
    ...
    assertAudioConfig(requestConfig, model, config.channelMode === "local");
    const instructions = config.audioInstructions.trim();

    try {
        const headers = await localServerHeaders();
        const response = await axios.post<Blob>(
            localGenerationUrl('audio', model),
            {
                model,
                input: prompt,
            ...
            { headers, responseType: "blob", signal: options?.signal },
        );
...
function assertAudioConfig(config: AiConfig, model: string, integrated = false) {
    if (!model) throw new Error(apiText("audioModelRequired"));
