...
async function requestPhantomTowerImage(config: AiConfig, prompt: string, references: ReferenceImage[] = [], options?: RequestOptions) {
    if (typeof window === "undefined") return null;
    const desktopWindow = window as Window & { phantomTowerServer?: { getToken: () => Promise<string> } };
    const requestConfig = resolveModelRequestConfig(config, config.model || config.imageModel);
    // The local server owns credentials, provider routing, records and safety
    // handling. It is intentionally used in both Electron and browser dev mode.
    const token = await desktopWindow.phantomTowerServer?.getToken?.().catch(() => "") || "";
    const referenceDataUrls = await Promise.all(references.map((image) => imageToDataUrl(image, { signal: options?.signal })));
    const response = await fetch("http://127.0.0.1:4317/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        // `model` is the human-visible name while `modelConfigId` is the
        // exact `apiId:modelId` returned by Sample Factory. The server uses
        // the latter to resolve the configured route and credentials.
        body: JSON.stringify({ model: requestConfig.model, modelConfigId: requestConfig.model, prompt, images: referenceDataUrls.filter(Boolean), n: Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1))), mode: references.length ? "image" : "text", quality: config.quality, size: config.size, format: "png" }),
        signal: options?.signal,
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload?.error?.message || payload?.error || apiText("requestFailed"));
    const images = parseImagePayload(payload);
    // /api/generate already writes the canonical generation record. Do not add
    // a canvas-only duplicate to the same history timeline.
    return images;
}

export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    const local = await requestPhantomTowerImage(config, prompt, [], options);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    const local = await requestPhantomTowerImage(config, prompt, references, options);
    if (local) return local;
    const requestConfig = resolveModelRequestConfig(config, config.model || config.imageModel);
    const n = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    if (config.channelMode === 'local') {
        const normalized = await Promise.all(messages.map(async (message) => ({
            ...message,
            content: typeof message.content === 'string' ? message.content : await Promise.all(message.content.map(async (part) => part.type === 'text' ? part : {
                ...part, image_url: { url: await imageToDataUrl({ url: part.image_url.url }, options) },
            })),
        })));
        const response = await fetch(localGenerationUrl('text', requestConfig.model), {
            method: 'POST',
            headers: { ...await localServerHeaders(), 'Content-Type': 'application/json' },
            body: JSON.stringify({ model: requestConfig.model, messages: withSystemMessage(requestConfig, normalized), reasoningEffort: config.reasoningEffort }),
            signal: options?.signal,
        });
        const payload = await response.json();
        if (!response.ok) throw new Error(readApiErrorMessage(payload) || apiText('requestFailed'));
        const answer = payload.text || apiText('noContent');
        onDelta(answer);
        return answer;
    }
