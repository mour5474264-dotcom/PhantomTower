...
import { localGenerationUrl, localServerHeaders } from './local-server';
import { nanoid } from "nanoid";

import i18n from "@canvas/i18n";
import { dataUrlToFile, readFileAsDataUrl } from "@canvas/lib/image-utils";
import { clampVideoSeconds, computeVideoSize, inferVideoRatio } from "@canvas/lib/media-size";
...
async function phantomTowerToken() {
    if (typeof window === "undefined") return "";
    const bridge = (window as Window & { phantomTowerServer?: { getToken?: () => Promise<string> } }).phantomTowerServer;
    return bridge?.getToken ? bridge.getToken().catch(() => "") : "";
}
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    const h3 = /^minimax-h3(?:-|$)/i.test(modelOptionName(selectedModel));
    const script = h3 ? undefined : resolveModelScript(config, selectedModel);
    if (script) return createPluginVideoTask(requestConfig, selectedModel, script, prompt, references, options);
    assertVideoConfig(requestConfig, requestConfig.model, config.channelMode === "local");
    if (!h3 && requestConfig.apiFormat === "gemini") return createGeminiVideoTask(requestConfig, selectedModel, prompt, references, options);
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    body.append("ratio", videoAspectRatio(config.size));
    for (const video of options?.videos || []) if (video.url?.startsWith('https://')) body.append('video_url[]', video.url);
    for (const audio of options?.audios || []) if (audio.url?.startsWith('https://')) body.append('audio_url[]', audio.url);
    ...
    try {
        const endpoint = localGenerationUrl('video', model);
        // Let the browser attach the multipart boundary for FormData.
        const headers = await localServerHeaders();
        const created = unwrapVideoResponse((await axios.post<ApiVideoResponse>(endpoint, body, { headers, signal: options?.signal })).data);
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const headers = await localServerHeaders();
        const video = unwrapVideoResponse((await axios.get<ApiVideoResponse>(localGenerationUrl('video', task.model, task.id), { headers, signal: options?.signal })).data);
        const url = videoResultUrl(video);
        if (video.status === "failed" || video.status === "error" || video.status === "cancelled") return { status: "failed", error: readApiErrorMessage(video.error) || apiText("videoGenerationFailed") };
        if (video.status === "completed" || video.status === "succeeded") {
            const content = await axios.get<Blob>(localGenerationUrl('video', task.model, task.id, true), { headers, responseType: "blob", signal: options?.signal });
            await assertVideoBlob(content.data);
            return { status: "completed", result: { blob: content.data } };
        }
        if (!video.status && url) return { status: "completed", result: await videoResultFromUrl(url, options) };
        return { status: "pending" };
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("videoTaskQueryFailed")), { cause: error });
    }
}
...
async function createGeminiVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    try {
        const created = unwrapEnvelope((await axios.post<ApiEnvelope<GeminiVideoOperation>>(localGenerationUrl('video', model), {
            instances: [instance],
            parameters: {
        ...
        }, { headers: await localServerHeaders(), signal: options?.signal })).data, apiText("noVideoTask"));
...
async function pollGeminiVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const headers = await localServerHeaders();
        const state = unwrapEnvelope((await axios.get<ApiEnvelope<GeminiVideoOperation>>(localGenerationUrl('video', task.model, task.id), { headers, signal: options?.signal })).data, apiText("videoTaskQueryFailed"));
        ...
        const response = await axios.get<Blob>(localGenerationUrl('video', task.model, task.id, true), { headers, responseType: 'blob', signal: options?.signal });
        await assertVideoBlob(response.data);
...
function assertVideoConfig(config: AiConfig, model: string, integrated = false) {
...
export function unwrapVideoResponse(payload: ApiVideoResponse) {
    const envelope = unwrapEnvelope(payload, apiText("noVideoTask")) as VideoResponse & {task_id?: string; data?: VideoResponse & {task_id?: string}};
    const video = envelope.data || envelope;
    return { ...video, id: video.id || video.task_id || '', status: video.status?.toLowerCase() };
}
