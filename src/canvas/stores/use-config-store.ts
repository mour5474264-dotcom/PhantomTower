...
export type ChannelModel = {
    /** Stable routing value sent to the Sample Factory generation API. */
    name: string;
    /** Human-readable name shown in selectors; falls back to `name`. */
    label?: string;
...
export const CONFIG_STORE_KEY = "phantom-tower:canvas_ai_config";
...
export function modelMatchesCapability(config: AiConfig, value: string, capability?: ModelCapability) {
    if (config.channelMode === "local") return config.models.includes(value);
...
export function resolveModelForCapability(config: AiConfig, currentModel: string | undefined, capability: ModelCapability) {
    ...
    // Integrated canvas must not revive bundled demo models when the active
    // Sample Factory workspace has no configured model.
    if (config.channelMode === "local") return "";
    return fallbackModel;
}

export function selectableModelsByCapability(config: AiConfig, capability?: ModelCapability) {
    // Exact parity with Sample Factory's creation workspace: no local
    // capability inference and no canvas-side filtering.
    if (config.channelMode === "local" || !capability) return config.models;
...
function isAiConfigReady(config: AiConfig, model: string) {
    ...
    const desktopBridge = typeof window !== "undefined" && Boolean((window as Window & { phantomTowerServer?: { getToken?: unknown } }).phantomTowerServer?.getToken);
    // In integrated mode keys stay in the PhantomTower server, never in canvas
    // storage. The server reports a useful configuration error if none is active.
    return Boolean(model.trim() && (config.channelMode === "local" || (channel.baseUrl.trim() && (channel.apiKey.trim() || desktopBridge))));
}
    ...
    persist(
        (set, get) => ({
            ...
            openConfigDialog: (shouldPromptContinue = false, configTab = "channels") => {
                if (get().config.channelMode === "local") {
                    if (window.parent !== window) window.parent.postMessage({ type: "phantom-tower:open-api-settings" }, window.location.origin);
                    else window.location.href = "./index.html#/apis";
                    return;
                }
...
export function normalizeChannelModels(models: Array<string | ChannelModel> | undefined): ChannelModel[] {
    ...
    for (const item of models || []) {
        ...
        result.push({ name, label, capability, script, ...(typeof item !== "string" && item.apiFormat ? { apiFormat: item.apiFormat } : {}) });
    }
    return result;
}
...
export function modelOptionLabel(config: AiConfig, value: string) {
    ...
    const label = model?.label || decoded.model;
...
export function resolveModelRequestConfig(config: AiConfig, value: string) {
    ...
    return {
        ...
        apiKey: channel.apiKey,
        apiFormat: findChannelModel(config, value)?.model.apiFormat || channel.apiFormat,
    };
}
