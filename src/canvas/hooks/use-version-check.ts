...
// This canvas is distributed as part of Sample Factory.  The upstream
// project's update endpoint is unrelated to this application's release
// channel and, in desktop builds, can produce failing requests on every load.
const updateChecksEnabled = import.meta.env.VITE_CANVAS_UPDATE_CHECK === "true";
...
export function useVersionCheck() {
    ...
    const checkLatestVersion = useCallback(async () => {
        if (!updateChecksEnabled) return false;
        try {
    ...
    const checkLatestRelease = useCallback(
        async (showMessage = false) => {
            if (!updateChecksEnabled) {
                if (showMessage) message.info("无限画布随样片工厂一起更新");
                return false;
            }
            setChecking(true);
            try {
