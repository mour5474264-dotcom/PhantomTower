...
async function fetchPluginSource(url: string) {
    ...
    if (parsed.protocol !== "https:" && !isLocalDev && parsed.protocol !== "file:") {
        throw new Error(i18n.t("canvas.pluginErrors.downloadFailed", { status: "仅支持 HTTPS 插件地址" }));
    }
...
async function loadLocalPlugins() {
    let urls: unknown;
    try {
        const response = await fetch("/canvas-app/plugins/index.json");
